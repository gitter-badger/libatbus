...
static void node_test_on_debug(const char* file_path, size_t line, const atbus::node& n, const atbus::endpoint* ep, const atbus::connection* conn, const char* fmt, ...) {
    CASE_MSG_INFO() << "[Log Debug][" << file_path << ":" << line << "] node=0x" << std::hex<< n.get_id() <<
        ", endpoint=0x" << std::hex << (NULL == ep ? 0 : ep->get_id()) <<
        ", connection=" << (NULL == conn ? "NULL" : conn->get_address().address)<< "\t\t";
