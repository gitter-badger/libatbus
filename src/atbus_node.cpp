...
namespace atbus {
    ...
    node::node(): id_(0), ev_loop_(NULL), static_buffer_(NULL){
    }
    ...
    void node::default_conf(conf_t* conf) {
        ...
        conf->msg_size = ATBUS_MACRO_MSG_LIMIT - detail::buffer_block::head_size(ATBUS_MACRO_MSG_LIMIT) - 16;
    ...
    int node::init(bus_id_t id, const conf_t* conf) {
        ...
        conf_.msg_size += detail::buffer_block::head_size(conf_.msg_size) + 16; // 预留crc32长度和vint长度
        static_buffer_ = detail::buffer_block::malloc(conf_.msg_size);

        ev_loop_ = NULL;

        // TODO 连接父节点
        if (!conf->father_address.empty()) {
    ...
    void node::iostream_on_recv_cb(const channel::io_stream_channel* channel, const channel::io_stream_connection* connection, int status, void* buffer, size_t s) {
    ...
    void node::iostream_on_connected(const channel::io_stream_channel* channel, const channel::io_stream_connection* connection, int status, void* buffer, size_t s) {
        // TODO 连接成功加入点对点传输池
        // TODO 发送注册协议
    }

    void node::iostream_on_disconnected(const channel::io_stream_channel* channel, const channel::io_stream_connection* connection, int status, void* buffer, size_t s) {  
        // TODO 移除相关的连接
        // TODO 移除相关的node信息
    }
